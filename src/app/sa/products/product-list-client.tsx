...
import {
  ...
  Loader2, Package, Power, LayoutGrid, LayoutList, FileSpreadsheet,
} from "lucide-react";
...
function EmptyState({ search, activeTab, colSpan }: { search: string; activeTab: "aktif" | "arsip"; colSpan?: number }) {
  const inner = (
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-white text-emerald-700 border border-emerald-200 px-4 py-2.5 rounded-xl font-black text-xs shadow-sm hover:bg-emerald-50 transition-all active:scale-95 shrink-0"
        >
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredProducts.length === 0 ? (
                  <EmptyState search={search} activeTab={activeTab} colSpan={5} />
                ) : (
      ...
      <ImportProductsModal open={isImportOpen} onClose={() => setIsImportOpen(false)} />
    </div>
  );
}
