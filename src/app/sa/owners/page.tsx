...
export default async function OwnersPage({
  ...
  // 1. Owner IDs — sumber tunggal app_users.is_owner (global) atau membership per-tenant.
  let allOwnerIds: string[];
  if (tenantId) {
    ...
    allOwnerIds = Array.from(new Set((assignedOwners ?? []).map((m) => m.user_id)));
  } else {
    const { data: ownerRows } = await supabaseAdmin
      .from("app_users")
      .select("id")
      .eq("is_owner", true);
    allOwnerIds = (ownerRows ?? []).map((r) => r.id);
  }
