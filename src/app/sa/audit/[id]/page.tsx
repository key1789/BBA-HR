...
function routeIdFromPathname(pathname: string): string {
  const match = pathname.match(/\/sa\/audit\/([^/?#]+)/i);
  return match?.[1]?.trim() ?? "";
}
...
export default async function AuditDetailPage({
  ...
  // Analyst hanya boleh akses apotek yang di-assign (satu sumber dgn server actions audit).
  if (auditTenantScopeError(session, tenantId)) return notFound();
    ...
    <AnimatedPage className="space-y-4">
      {kpiMissing && (
        <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 mb-4">
          ...
          <Link
            href={`/sa/branches/${tenantId}?tab=kpi&month=${month}&year=${year}`}
