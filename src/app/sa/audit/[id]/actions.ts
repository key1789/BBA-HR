...
async function assertAuditMutationAccess() {
  ...
  if (session?.activeMembership?.role !== "super_admin_bba") {
    ...
    return { error: "Hanya super admin Apotrik yang dapat mengubah data audit." } as const;
  }
...
async function revalidateAuditDetailForMonthlyAuditId(supabaseAdmin: ReturnType<typeof createAdminClient>, auditId: string) {
  ...
  if (data?.tenant_apotek_id) {
    revalidatePath(`/sa/audit/${data.tenant_apotek_id}`);
  }
}
...
export async function toggleCrewLockAction(auditId: string, userId: string, currentStatus: boolean) {
  ...
  const scopeErr = auditTenantScopeError(access.session, String(mainAudit.tenant_apotek_id));
...
export async function updateCrewAuditAction(
  ...
  // Validasi nilai penilaian (server-side, bukan hanya client): skor 0..100;
  // bba_adjustment (uang) dibatasi ±10jt agar tak salah ketik jadi nominal ekstrem.
  for (const key of ["analyst_score", "internal_review_score", "customer_review_score"] as const) {
    const v = updates[key];
    if (v !== undefined && (typeof v !== "number" || !Number.isFinite(v) || v < 0 || v > 100)) {
      return { error: "Skor harus berupa angka 0 sampai 100." };
    }
  }
  if (updates.bba_adjustment !== undefined) {
    const v = updates.bba_adjustment;
    if (typeof v !== "number" || !Number.isFinite(v) || v < MIN_ADDON_NOMINAL || v > MAX_ADDON_NOMINAL) {
      return { error: "Penyesuaian BBA harus di rentang -10.000.000 sampai 10.000.000." };
    }
  }
...
export async function submitAuditForReviewAction(auditId: string) {
  ...
  const scopeErr = auditTenantScopeError(access.session, String(auditRow.tenant_apotek_id));
  if (scopeErr) return { error: scopeErr };
...
export async function upsertMonthlyAddonAppraisalAction(input: {
  ...
  const scopeErr = auditTenantScopeError(access.session, input.tenantApotekId);
  if (scopeErr) return { error: scopeErr };
  ...
  revalidatePath(`/sa/audit/${input.tenantApotekId}`);
  return { success: true };
}
...
type PayrollDraftItem = {
  ...
  customAdjustments: Array<{ name: string; amount: number; type: string; basis?: string }>;
  kpiBonus: number;
  productBonus: number;
  adjustment: number;
...
export async function savePayrollDraftAction(input: {
  ...
  // ── Guard: tenant-scope aktor (analyst hanya cabang yang di-assign) ──────────
  const scopeErr = auditTenantScopeError(access.session, input.tenantId);
  if (scopeErr) return { error: scopeErr };
  ...
  // ── BONUS = sumber kebenaran server (bukan angka dari client) ────────────────
  // Segarkan rapor dulu agar monthly_appraisals mutakhir, lalu baca bonus dari sana.
  const syncRes = await syncMonthlyAppraisalsForAuditPeriod(supabaseAdmin, {
    auditId: input.auditId,
    actorUserId: user.id,
  });
  if (syncRes.error) {
    return { error: `Gagal menyegarkan rapor sebelum menyimpan payroll: ${syncRes.error}` };
  }

  const { data: apprRows, error: apprErr } = await supabaseAdmin
    .from("monthly_appraisals")
    .select("crew_user_id, auto_bonus_accountability, product_fokus_bonus, bba_adjustment")
    .eq("tenant_apotek_id", input.tenantId)
    .eq("period_month", input.month)
    .eq("period_year", input.year);
  if (apprErr) {
    return { error: "Gagal membaca rapor bulanan untuk payroll." };
  }
  const bonusByUser = new Map<string, { kpi: number; produk: number; adj: number }>();
  for (const r of apprRows ?? []) {
    bonusByUser.set(String(r.crew_user_id), {
      kpi: Number(r.auto_bonus_accountability ?? 0),
      produk: Number(r.product_fokus_bonus ?? 0),
      adj: Number(r.bba_adjustment ?? 0),
    });
  }

  // ── Validasi input gaji (admin boleh override, tapi harus waras) ─────────────
  for (const item of input.items) {
    const nums = [item.baseSalary, item.posAllowance, item.mealAllowance, item.transAllowance, item.bpjsDeduction];
    if (nums.some((v) => !Number.isFinite(v) || v < 0)) {
      return { error: "Nilai gaji, tunjangan, atau potongan tidak boleh negatif." };
    }
    if (item.daysWorked !== null && (!Number.isFinite(item.daysWorked) || item.daysWorked < 0 || item.daysWorked > lastDay)) {
      return { error: `Jumlah hari kerja harus antara 0 dan ${lastDay}.` };
    }
    for (const adj of item.customAdjustments ?? []) {
      if (!Number.isFinite(Number(adj.amount)) || Number(adj.amount) < 0) {
        return { error: "Nominal penyesuaian tidak boleh negatif." };
      }
      if (adj.type !== "addition" && adj.type !== "deduction") {
        return { error: "Tipe penyesuaian tidak valid." };
      }
    }
    // Hari Masuk KOSONG (null) sementara ada komponen HARIAN (>0) → tunjangan harian
    // akan tersimpan 0 (underpay diam-diam vs preview). Wajibkan isi Hari Masuk dulu.
    // Catatan: daysWorked = 0 (eksplisit) tetap sah — crew memang tak masuk bulan itu.
    const hasDailyDependent =
      item.mealAllowance > 0 ||
      item.transAllowance > 0 ||
      (item.customAdjustments ?? []).some(
        (a) => String(a.basis ?? "monthly").toLowerCase() === "daily" && Number(a.amount) > 0,
      );
    if (item.daysWorked === null && hasDailyDependent) {
      return {
        error:
          "Hari Masuk wajib diisi untuk karyawan yang punya tunjangan harian (makan/transport/penyesuaian harian) sebelum menyimpan payroll.",
      };
    }
  }
  ...
  const itemRows = input.items.map((item) => {
    ...
    // Basis "daily" dikali hari masuk (dw); "monthly"/default dikali 1. Bila dw null → daily = 0 (seperti makan/transport).
    // Pengali dari SATU sumber: customAdjMultiplier (audit-utils). days = dw ?? 0.
    const customAdds = (item.customAdjustments ?? [])
      .filter((a) => a.type === "addition")
      .reduce((s, a) => s + Math.max(0, Number(a.amount) || 0) * customAdjMultiplier(a, dw ?? 0), 0);
    ...
    const srvBonus = bonusByUser.get(item.userId) ?? { kpi: 0, produk: 0, adj: 0 };
    const bonusTotal = srvBonus.kpi + srvBonus.produk + srvBonus.adj;
    ...
    const configSnapshot = {
      ...
      bonus_from_audit: {
        kpi:        srvBonus.kpi,
        produk_fokus: srvBonus.produk,
        adjustment: srvBonus.adj,
        total:      bonusTotal,
      },
      totals: {
  ...
  revalidatePath(`/sa/audit/${input.tenantId}`);
  revalidatePath("/sa/payroll");
  return {
    success: true,
