...
import { productFokusEarnedForConfig, computeProductFokusBonusTotalForUser } from "@/lib/produk-fokus-core";
...
export type PayrollCustomAdj = { id?: string; name?: string; type?: string; amount?: unknown; basis?: string };
...
// Perhitungan bonus produk fokus tinggal di modul murni (satu sumber, bisa di-unit-test).
export { productFokusEarnedForConfig, computeProductFokusBonusTotalForUser };
...
export function netFromCustomAdjustments(raw: unknown, days: number): number {
  let net = 0;
  for (const o of parseCustomAdjustmentsArray(raw)) {
    if (BPJS_CUSTOM_ADJ_TYPES.has(String(o.type ?? "").toLowerCase())) continue;
    const amt = Math.abs(Number(o.amount ?? 0)) * customAdjMultiplier(o, days);
 ...
 *  Item basis "daily" dikali `days`. Item BPJS diexclude — sudah masuk via bpjsDeduction. */
export function customAdjustmentTableRows(raw: unknown, days: number): { key: string; label: string; val: number; tone: string }[] {
  return parseCustomAdjustmentsArray(raw)
    .filter((o) => !BPJS_CUSTOM_ADJ_TYPES.has(String(o.type ?? "").toLowerCase()))
    .map((o, idx) => {
    const isDaily = String(o.basis ?? "monthly").toLowerCase() === "daily";
    const amt = Math.abs(Number(o.amount ?? 0)) * (isDaily ? days : 1);
    const type = String(o.type ?? "addition").toLowerCase();
    const val = type === "deduction" ? -amt : amt;
    const name = String(o.name ?? "").trim() || "Penyesuaian";
    const suffix = type === "deduction" ? "pengurangan" : "tambahan";
    return {
      key: `payroll-adj-${String(o.id ?? idx)}-${idx}`,
      label: isDaily ? `${name} (${suffix} · ×${days} hari)` : `${name} (${suffix})`,
      val,
