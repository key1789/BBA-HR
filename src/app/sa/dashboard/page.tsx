...
function dashboardHref(opts: {
  ...
  q.set("tab", opts.tab);
  return `/sa/dashboard?${q.toString()}`;
}
...
export default async function BbaControlDashboardPage({
  ...
  if (!active || active.role !== "super_admin_bba") {
    return <p className="text-sm text-slate-600">Control dashboard hanya untuk super admin Apotrik.</p>;
  }
    ...
    return (
      <AnimatedPage className="space-y-4">
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h1 className="text-2xl font-black text-slate-900 uppercase">Apotrik Dashboard</h1>
  ...
  const auditHrefThisPeriod = `/sa/audit?month=${month}&year=${year}`;
  ...
  const [
    ...
    supabase
      .from("daily_submissions")
      .select(
        "tenant_apotek_id, submission_date, omzet_total, transaction_total, product_total, rejected_customer_total, rejected_medicine_total, user_id, user:user_id(full_name)",
      )
      .in("tenant_apotek_id", scopedTenantIds)
  ...
  const branchHealthRows: BranchHealthRow[] = tenantRows.map((row) => ({
    ...
    detailHref: `/sa/audit?tenant=${row.tenantId}&month=${month}&year=${year}`,
  }));
  ...
  const salesBranchSummaries: SalesBranchSummary[] = [];
