...
export async function createBranchAction(prevState: any, formData: FormData) {
  ...
  if (branchError) {
    const isDup =
      (branchError as { code?: string }).code === "23505" ||
      /unique|duplicate/i.test(branchError.message ?? "");
    if (isDup) {
      return { error: `Kode apotek "${code}" sudah dipakai. Gunakan kode lain.` };
    }
...
export async function resetTrialBranchAction(branchId: string) {
  ...
  for (const table of operationalTables) {
    ...
    if (error) {
      ...
      // Tabel tidak ada di skema ini -> tidak ada data yang perlu dibersihkan; ini BUKAN kegagalan.
      // (Kalau kelak tabelnya dibuat, reset otomatis ikut membersihkannya.)
      const missingTable =
        code === "42P01" ||
        code === "PGRST205" ||
      ...
      if (!missingTable) {
        // Kegagalan asli (mis. FK / permission) — baru dilaporkan.
