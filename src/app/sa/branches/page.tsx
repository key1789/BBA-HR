...
import { isKpiConfigReady } from "@/lib/kpi-v2/utils";
...
export default async function BbaBranchesPage() {
  ...
  const [{ data: allBranches }, { data: ownerRows }] = await Promise.all([
    supabaseAdmin
      .from("tenant_apotek")
      .select(`
        *,
  ...
  // Scope daftar cabang: global lihat semua; super_admin_bba non-global hanya cabang assigned.
  const session = await getSessionContext();
  const branches = session?.isGlobalSuperAdmin
    ? allBranches
    : (allBranches ?? []).filter((b: { id: string }) => bbaCanAccessTenant(session, b.id));
  ...
  const [{ data: kpiConfigs }, { data: masterShiftsAll }] = branchIds.length > 0
    ? await Promise.all([
        supabaseAdmin.from("kpi_configs").select("tenant_apotek_id, target_omzet, bonus_config_v2").in("tenant_apotek_id", branchIds),
        supabaseAdmin.from("master_shifts").select("tenant_apotek_id").in("tenant_apotek_id", branchIds),
      ])
    : [{ data: [] as { tenant_apotek_id: string; target_omzet: number | null; bonus_config_v2: unknown }[] }, { data: [] as { tenant_apotek_id: string }[] }];

  // Hanya hitung cabang yang KPI-nya benar-benar terkonfigurasi (bukan baris bootstrap target 0).
  const branchesWithKpi = new Set(
    (kpiConfigs ?? []).filter((k) => isKpiConfigReady(k)).map((k) => k.tenant_apotek_id),
  );
