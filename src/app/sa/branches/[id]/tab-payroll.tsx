...
import {
  ...
  AlertCircle, ShieldCheck, UserCog, Crown, Banknote, Trash2,
} from "lucide-react";
...
// ─── Baris penyesuaian kustom: nama (besar) + nominal + basis (Bulanan / per hari) ──
function AdjustmentRow({
  item, accent, workDays, onUpdate, onRemove,
}: {
  item: CustomAdjustment;
  accent: "emerald" | "rose";
  workDays: number;
  onUpdate: (patch: Partial<CustomAdjustment>) => void;
  onRemove: () => void;
}) {
  const isDaily = item.basis === "daily";
  const amtBorder = accent === "emerald"
  ...
  return (
    <div className="rounded-xl border border-slate-100 bg-slate-50/40 p-2.5 space-y-2">
      <input
        placeholder={accent === "emerald" ? "Nama penambahan (mis. Uang Lembur)" : "Nama pengurangan (mis. Kasbon)"}
        value={item.name}
      ...
      <div className="flex items-center gap-2">
        <CurrencyInput value={item.amount} onChange={(val) => onUpdate({ amount: val })}
          className={cn("flex-1 min-w-0 rounded-lg border px-2.5 py-2 text-sm font-black outline-none transition-all", amtBorder)} />
        <div className="flex shrink-0 rounded-lg border border-slate-200 overflow-hidden text-[9px] font-black uppercase">
          <button type="button" onClick={() => onUpdate({ basis: "monthly" })}
            className={cn("px-2.5 py-2 transition-colors", !isDaily ? "bg-slate-700 text-white" : "bg-white text-slate-400 hover:text-slate-600")}>Bulanan</button>
          <button type="button" onClick={() => onUpdate({ basis: "daily" })}
            className={cn("px-2.5 py-2 transition-colors", isDaily ? "bg-amber-500 text-white" : "bg-white text-slate-400 hover:text-amber-600")}>/ hari</button>
        </div>
        <button type="button" onClick={onRemove} className="text-rose-300 hover:text-rose-500 transition-colors shrink-0">
          <Trash2 size={14} />
        </button>
      </div>
      {isDaily && item.amount > 0 && (
        <p className="text-[9px] text-amber-600 font-bold">× {workDays} hari ≈ {IDR.format(item.amount * workDays)} · aktual ikut hari masuk</p>
      )}
    </div>
  );
}
            ...
            {customAdj.filter((c) => c.type === "addition").length === 0 ? (
              ...
              <div className="space-y-2">
                {customAdj.filter((c) => c.type === "addition").map((item) => (
                  <AdjustmentRow key={item.id} item={item} accent="emerald" workDays={workDays}
            ...
            {customAdj.filter((c) => c.type === "deduction").length === 0 ? (
              ...
              <div className="space-y-2">
                {customAdj.filter((c) => c.type === "deduction").map((item) => (
                  <AdjustmentRow key={item.id} item={item} accent="rose" workDays={workDays}
                    onUpdate={(patch) => updateAdj(item.id, patch)} onRemove={() => removeAdj(item.id)} />
                ))}
              </div>
            )}
