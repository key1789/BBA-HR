...
export default async function BranchDetailPage({ params, searchParams }: { params: Promise<{ id: string }>, searchParams: Promise<{ month?: string, year?: string }> }) {
  ...
  // Scope cabang: super_admin_bba non-global hanya boleh membuka cabang yang di-assign.
  if (!bbaCanAccessTenant(session, id)) notFound();
  ...
  // Halaman /sa/* hanya untuk super_admin_bba (layout) — service read aman & konsisten.
