...
import {
  ...
  clearExpiredStaffInvitationsAction,
} from "./actions";
...
function PendingInvitations({ branchId }: { branchId: string }) {
  ...
  const handleDelete = async (inv: any) => {
    if (!window.confirm(`Hapus undangan untuk ${inv.full_name || inv.email}?`)) return;
    setDeletingId(inv.id);
    const result = await cancelStaffInvitationAction(inv.id);
    if (result.error) {
      toast.error(result.error);
    } else if (result.success) {
      toast.success(result.message ?? "Undangan dihapus.");
      await load();
    }
    setDeletingId(null);
  };

  const handleClearExpired = async (count: number) => {
    if (!window.confirm(`Bersihkan ${count} undangan kedaluwarsa di cabang ini?`)) return;
    setIsClearing(true);
    const result = await clearExpiredStaffInvitationsAction(branchId);
    if (result.error) {
      toast.error(result.error);
    } else if (result.success) {
      toast.success(result.message ?? "Undangan kedaluwarsa dibersihkan.");
      await load();
    }
    setIsClearing(false);
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        {expired.length > 0 && (
          <button
            onClick={() => handleClearExpired(expired.length)}
            disabled={isClearing}
            className="ml-auto flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-rose-600 bg-rose-50 border border-rose-200 rounded-lg hover:bg-rose-100 transition-colors disabled:opacity-50"
          >
                  ...
                  <td className="py-3.5 px-5 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center justify-center w-8 h-8 rounded-lg text-slate-400 border border-slate-200 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition-colors disabled:opacity-50"
                      >
