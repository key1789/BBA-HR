...
export function BranchDetailClient({
  ...
  const absensiShiftAddon      = addons.find((a: any) => a.addon_key === 'absensi_shift');
  const isAbsensiEnabled       = absensiShiftAddon?.is_enabled ?? false;
  const allowOwnerSchedule     = Boolean((absensiShiftAddon?.settings as any)?.allow_owner_schedule);
  const allowAdminSchedule     = Boolean((absensiShiftAddon?.settings as any)?.allow_admin_schedule);
          ...
          >
            ...
            {activeTab === "operasional" && (
              <TabOperasional
                ...
                defaultWorkingDays={Number(kpiConfigV2?.global?.default_working_days) || 26}
              />
            )}
            {activeTab === "addons" && (
              <TabAddons
