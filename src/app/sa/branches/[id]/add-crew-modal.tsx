...
export function AddCrewModal({ isOpen, onClose, branchId, branchName, branchCode = "" }: Props) {
  ...
  return (
    <AnimatedModal isOpen={isOpen} onClose={onClose} title="Tambah Crew">
      {/* Tab selector */}
      <div className="mb-2 flex bg-slate-100 p-1 rounded-xl">
        <button
      ...
      {/* Penjelasan tab — di LUAR tombol (tombol tak boleh berisi tombol/tooltip-button). */}
      <p className="mb-5 px-1 text-[11px] leading-relaxed text-slate-400">
        {activeTab === "invite"
          ? "Buat akun baru. Karyawan akan menerima link undangan untuk set password."
          : "Tambahkan user yang sudah memiliki akun di sistem ke cabang ini."}
      </p>
