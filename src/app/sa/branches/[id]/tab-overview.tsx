...
export function TabOverview({
  ...
  // ── KPI Data ──
  const isKpiReady    = isKpiConfigReady(kpi);
  ...
  const activeUnsetAddonsCount = activeAddons.filter(
    (a) => !isAddonConfigured(a.addon_key, a.settings),
  ).length;
            ...
            <GlassCard className="p-6" variant="light">
              ...
              <div className="mt-5 pt-5 border-t border-slate-100">
                ...
                >
                  <select
                    name="newOwnerId"
                    className="flex-1 min-w-0 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:bg-white transition-all"
                    defaultValue=""
                    required
                  >
                  ...
                  <button
                    type="submit"
                    className="shrink-0 px-4 py-2 bg-amber-100 text-amber-700 hover:bg-amber-200 rounded-xl text-xs font-black uppercase tracking-widest transition-colors flex items-center justify-center gap-1.5 whitespace-nowrap"
                  >
