...
import { createOwnerPasswordResetLinkAction } from "./actions";
...
export function ResendAccessModal({ isOpen, onClose, owner }: Props) {
  ...
  const handleSendWA = () => {
    let phoneStr = owner.phone || "";
    if (phoneStr.startsWith("0")) {
      phoneStr = "62" + phoneStr.substring(1);
    }

    const text = `Halo ${owner.full_name},

Berikut link untuk mengganti password akun BBA Portal Anda sebagai Owner:
        ...
        <div className="bg-sky-50 border border-sky-100 rounded-xl p-4 flex gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-sky-700/80 mt-1">
              Buat link reset password untuk {owner.full_name}. Link bisa dikirim via WA tanpa membagikan password plaintext.
            </p>
          </div>
        </div>
