...
export function AcceptStaffInvitationClient({ invitation, token }: Props) {
  const [state, action, isPending] = useActionState(completeStaffInvitationAction, null);
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
        <div className="bg-sky-600 p-8 text-white text-center">
          ...
          <h1 className="text-2xl font-black tracking-tight">Aktivasi Akun Pegawai</h1>
          <p className="text-sky-100 mt-2 text-sm opacity-90">Set password untuk menyelesaikan pendaftaran akun Anda.</p>
        </div>
        ...
        <form action={action} className="p-8 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold text-slate-700">
                {invitation.role === "admin_apotek" ? "Admin Apotek" : "Crew"}
              </div>
            </div>
