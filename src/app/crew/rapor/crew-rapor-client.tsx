...
import {
  ...
  Trophy, Star, Wallet, Award, BarChart2, Zap,
} from "lucide-react";
...
type SnapshotRow = { userId: string; name: string; omzet: number; atv: number; atu: number; sarpPct: number; lateFlag: number };
type RunningRow  = { userId: string; name: string; omzet: number; atv: number; atu: number; days: number };
...
type Props = {
  ...
  thpData: any;
  // Leaderboard
  teamLeaderboard: SnapshotRow[];
  runningLeaderboard: RunningRow[];
...
function RaporTab(props: Props) {
  const {
    ...
    autoBonus, addonManual, bbaAdj, calcBreakdown, approvedCount,
  ...
  const hasAnything = hasBonus || hasSnapshot || hasRunning;
  ...
  // Performa yang ditampilkan: snapshot kalau ada, fallback ke running
  ...
  const capaianColor  = capaianPct == null ? "sky"
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Target vs Capaian — progress card */}
      {(hasRunning || hasSnapshot) && target != null && target > 0 && (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-5 pt-4 pb-3">
            <div className="flex items-center justify-between mb-1">
              ...
              {personalTarget && teamTarget && personalTarget !== teamTarget ? (
                <p className="text-[9px] font-bold text-slate-400">Target individu</p>
              ) : (
                <p className="text-[9px] font-bold text-slate-400">Target tim</p>
              )}
            </div>
            ...
            <div className="flex items-end justify-between mt-3 mb-2">
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-0.5">Omzet Berjalan</p>
                <p className="text-2xl font-black text-slate-900 leading-none">{IDR.format(perfOmzet)}</p>
              </div>
              <div className="text-right">
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-0.5">Target</p>
                <p className="text-sm font-black text-slate-500">{IDR.format(target)}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <p className="text-[10px] font-bold text-slate-400">
                {perfDays} hari input approved
              </p>
            </div>

            {perfIsRunning && (
              <p className="text-[9px] font-bold text-amber-600 mt-2 bg-amber-50 rounded-xl px-3 py-1.5">
      ...
      {/* Performa & Capaian */}
      {(hasSnapshot || hasRunning) && (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <BarChart2 size={14} className="text-sky-600" />
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Performa</p>
            </div>
            {perfIsRunning && (
          ...
          <div className="p-4 grid grid-cols-2 gap-3">
            <StatChip label="Omzet" value={IDR.format(perfOmzet)} />
            ...
            {!useSnapshot && hasRunning && runningPersonal && (
              <StatChip label="Transaksi" value={NUM.format(runningPersonal.trx)} />
            )}
            <StatChip label="Input Approved" value={`${NUM.format(perfDays)} hari`} />
            {useSnapshot && mySnapshot && (
              <StatChip
                label="Late Flag"
          ...
          <div className="p-4 space-y-4">
            ...
            {peerRatingAvg !== null && (
              <div className="flex items-center justify-between">
                ...
                <p className="text-xl font-black text-slate-900">{peerRatingAvg.toFixed(1)}<span className="text-sm font-bold text-slate-400"> /5</span></p>
              </div>
            )}
...
function PeringkatTab(props: Props) {
  const { teamLeaderboard, runningLeaderboard, currentUserId, personalTarget, teamTarget, month, year } = props;

  // Pakai snapshot kalau ada, fallback ke running
  const isRunning = teamLeaderboard.length === 0 && runningLeaderboard.length > 0;
  const isEmpty   = teamLeaderboard.length === 0 && runningLeaderboard.length === 0;
  ...
  // Normalisasi ke format umum
  const rows: { userId: string; name: string; omzet: number; atv: number; atu: number; extra: string }[] = isRunning
    ? runningLeaderboard.map(r => ({
        userId: r.userId,
        name:   r.name,
        omzet:  r.omzet,
        atv:    r.atv,
    ...
    : teamLeaderboard.map(r => ({
        ...
        atv:    r.atv,
        atu:    r.atu,
        extra:  `SARP ${pct(r.sarpPct)}`,
      }));

  const podium = rows.slice(0, 3);
  const rest   = rows.slice(3);
  const myRank = rows.findIndex(r => r.userId === currentUserId) + 1;
  const target = personalTarget ?? teamTarget;
    ...
    <div className="space-y-4">
      ...
      {isRunning && (
        <div className="flex items-center gap-2.5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
          <Lock size={15} className="shrink-0 text-amber-600" />
          <p className="text-[11px] font-bold text-amber-800">
            Data berjalan dari input harian — diurutkan omzet, belum final
          </p>
        </div>
      )}

      {/* Posisi saya + progress ke target */}
      {myRank > 0 && (
        <div className="bg-sky-50 border border-sky-200 rounded-2xl px-4 py-3 space-y-2">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[11px] font-bold text-sky-800">
              Posisi kamu: <span className="font-black">#{myRank}</span> dari {rows.length} crew
            </p>
          </div>
          {target != null && target > 0 && (() => {
            const myOmzet = rows[myRank - 1]?.omzet ?? 0;
            const capaian = Math.min(150, (myOmzet / target) * 100);
            return (
              <div>
                <div className="flex justify-between text-[9px] font-bold text-sky-700 mb-1">
                  <span>{IDR.format(myOmzet)}</span>
                  <span>{capaian.toFixed(1)}% dari target</span>
                </div>
                <div className="h-2 bg-sky-100 rounded-full overflow-hidden">
                  <div
                    className={cn("h-full rounded-full", capaian >= 100 ? "bg-emerald-500" : capaian >= 75 ? "bg-sky-500" : capaian >= 50 ? "bg-amber-500" : "bg-rose-500")}
      ...
      <div className="space-y-2">
        {podium.map((item, idx) => {
          ...
          return (
            <div key={item.userId} className={cn(
              "flex items-center gap-3 p-3.5 rounded-2xl border",
              m.bg, m.border,
              isMe && "ring-2 ring-sky-400 ring-offset-1",
            )}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0">{m.emoji}</div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-black text-slate-800 truncate">
                  {item.name}
                  {isMe && <span className="ml-1.5 text-[9px] font-black text-sky-600 bg-sky-100 px-1.5 py-0.5 rounded-md">KAMU</span>}
                </p>
                <p className={cn("text-[10px] font-black mt-0.5", m.text)}>{IDR.format(item.omzet)}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="text-[9px] text-slate-400 font-bold">{item.extra}</p>
                <p className="text-[9px] text-slate-400 mt-0.5">ATV {IDR.format(item.atv)}</p>
              </div>
            </div>
          );
        })}
          ...
          {rest.map((item, idx) => {
            ...
            return (
              <div key={item.userId} className={cn(
                ...
                <span className="text-[11px] font-black text-slate-300 w-5 text-center shrink-0">{idx + 4}</span>
                <div className="flex-1 min-w-0">
                  <p className={cn("text-xs font-black truncate", isMe ? "text-sky-700" : "text-slate-700")}>
                    {item.name}
                    {isMe && <span className="ml-1.5 text-[9px] font-black text-sky-600 bg-sky-100 px-1.5 py-0.5 rounded-md">KAMU</span>}
                  </p>
                  <p className="text-[9px] text-slate-400 font-bold mt-0.5">{item.extra} · ATV {IDR.format(item.atv)}</p>
                </div>
                <p className="text-[10px] font-black text-emerald-600 shrink-0">{IDR.format(item.omzet)}</p>
              </div>
            );
          })}
        </div>
      ...
      <p className="text-center text-[10px] text-slate-400 font-medium">
        ...
        {isRunning ? " · Data berjalan, belum final" : " · Data tersinkronisasi"}
      </p>
    </div>
  );
}
